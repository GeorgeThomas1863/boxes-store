...
describe("email HTML content (via sendOrderConfirmationEmails)", () => {
  ...
  it("formats order date in Eastern time: UTC midnight-minus-1min renders as previous day", async () => {
    // 2026-04-15T03:59:00.000Z = April 14 at 11:59 PM EDT (UTC-4), not April 15
    const etOrder = { ...mockOrder, orderDate: "2026-04-15T03:59:00.000Z" };
    const html = await getBuyerHtml(etOrder);
    expect(html).toContain("April 14, 2026");
    expect(html).not.toContain("April 15, 2026");
  });
});
